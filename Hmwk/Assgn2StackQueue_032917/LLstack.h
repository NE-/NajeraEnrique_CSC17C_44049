/*
 * File: LLstack.h
 * Author: Najera Enrique
 * Date: 29 March 2017
 * Purpose: Create stack from linked list (LIFO)
 */

#ifndef LLSTACK_H
#define LLSTACK_H

// System Libraries
#include <iostream>
#include <iomanip>

using namespace std;

// User Libraries
#include "link.h"

// Start template class LLstack
template <class T>
class LLstack{
    public:
        // Constructor
        LLstack(){
            lnkList = new Link<T>;   // Create new Link
            lnkList->data = 0;       // Set to 0 for EXIT_SUCCESS
            lnkList->linkPtr = NULL; // Set to NULL
        }
        // Destructor
        ~LLstack(){
            if(lnkList==NULL)return;            //Empty List? then leave
            do{
                Link<T> *temp=lnkList->linkPtr; //Traverse the list
                delete lnkList;                 //Delete the front of the list
                lnkList=temp;                   //Set the new front of the list
            }while(lnkList!=NULL);              //Stop when we reach the end.
        }
        
        // Function Prototypes
        void prntStack(int); // Prints stack elements
        void push(T);        // Pushes type into stack
        T pop();             // Pops first element (LIFO)
        
    private:
        Link<T> *lnkList;
};// End template class LLstack

template <class T>
void LLstack<T>::prntStack(int perLine){
    int colCnt=0;                //Column counter
    Link<T> *next=lnkList;       //Start at the front of the list
    cout<<endl;                  //Put the beginning on a new line
    do{
        cout<<setw(5)<<next->data<<" ";   //Print the link
        if(colCnt++%perLine==(perLine-1))cout<<endl;
        next=next->linkPtr;      //Go to the next link
    }while(next!=NULL);          //Stop when your at the end
    cout<<endl;
}

// Start method push
template <class T>
void LLstack<T>::push(T val){
    Link<T> *newNode = new Link<T>; // Create new node

    newNode->data = val;            // Fill data with argument
    newNode->linkPtr = lnkList;     // Fill new address
    lnkList = newNode;              // Add to front
}// End method push

// Start method pop
template <class T>
T LLstack<T>::pop(){
    Link<T> *pop  = lnkList; // Holder for lnkList
    T tempVal = 0;           // Holds data before deletion
    
    // If nothing in list, output error and return 0
    if (lnkList->linkPtr == NULL)
        cout << "List is empty";
    else {
        tempVal = pop->data;    // Get data before deletion
        lnkList = pop->linkPtr; // Hold this address
        delete pop;             // Delete head
    }
    
    // Return data before deletion
    return tempVal;
}// End method pop

#endif /* LLSTACK_H */
